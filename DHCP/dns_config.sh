...
IP="192.168.10.10"              #IP ESTATICO DO SERVIDOR DNS
...
# Inclui fail2ban para cumprir o requisito de segurança
...
options {
    ...
    allow-transfer { none; }; // Restringir transferência de zona (Requisito de Segur>
    recursion yes;
...
// Configuração da Zona Direta (CORRIGIDO: usa DOMAIN FQDN)
zone "$DOMAIN" IN {
    type primary;
    file "$DOMAIN.db";
    allow-update { none; };
...
named-checkzone $DOMAIN /var/named/$DOMAIN.db || { echo "Erro na zona direta. Abortan>
named-checkzone $REVERSE_PREFIX.in-addr.arpa /var/named/$REVERSE_PREFIX.db || { echo >
